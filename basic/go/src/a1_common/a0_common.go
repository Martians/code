...
## 主要特点
 	编译快、执行快
	格式标准化、代码风格强制统一; 格式不对无法编译，类似于python
...
## 语言
	## 语法
		丰富的语法结构：switch 任意类型：字符串、多个if-else（类似于scala，高于java、python）
		引入了函数式编程的概念，不可变性（不可变的有：）
		变量定义上，进行了类型推导
		更多的内置类型；有nil值
...
## 对比
	print函数不够方便
	只有main可以执行，其他package都是lib，区分开来（类似python，模块查找时的一些原则）
	局部变量也可以取地址（编译器自行决定，是在堆还是在栈上分配内存）
	没有隐含的this指针（类似于python、rust）
	有原生字符串
	对集合的操作函数，是统一函数，而不是每个集合类内部的成员函数
	有nil存在，相当于null（与scala区分开来）
