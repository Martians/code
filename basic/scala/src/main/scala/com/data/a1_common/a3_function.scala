...
/**
  * 小结：
  *     1. 函数文本、函数值的区别，函数值才可以赋值给变量
  *     2. 简化函数调用的方法：目标类型化、占位符、偏应用函数
  *     3. 闭包（写flink代码时严重依赖于闭包）、重复参数、尾递归
  *     4. 新建控制结构：curry（；最后一个参数像控制结构一样使用）、叫名参数（延迟评估）
  *     5. 偏应用函数（已经指定了部分参数；特例：未指定所有参数，可用于转换文本到函数值，变成一个对象）
  *
  * 设计：
  ...
  *
  * 内部：
  *     Scala 中的函数值是一个完整的 - 对象，Scala 中的函数其实就是继承了 Trait （FunctionN）的类的对象
  *
  * 说明：
  *     1. 方法的形参，默认都是val的，不能够重新赋值
  *     2. 方法与函数：
  *             如果去掉了, 那么就是Unit返回值，必然是为了产生副作用，惯例上，调用时带()
  *             函数可作为一个参数传入到方法中，而方法不行，通过偏应用函数的方式转换为函数
  */
object a3_function extends App {
    ...
    /**
      ...
      *     3. 函数值是“关闭”这个行动的最终产物，得到的函数值中，将包含指向捕获变量的参考
      *
      */
    ...
    /**
      * 尾递归 (P123)
      *     当函数体执行的最后一件是事情，就是调用自己的函数时，时为递归
      *     抛出异常之后，可以看到例子中，堆栈部分并不是处于层层调用 bang 中，编译器进行了代码的转化
      *
      *     设计建议：
      *         1）尽量用递归代替循环
    ...
    def operator_abstract(): Unit = {
        ...
        /**
          ...
          *        filesEnding：将一个的高阶函数，传入给filesMatching，得到其中一种语义
          *
        ...
        /**
          * Curry
          *     目的：让控制抽象，感觉像是语言的扩展；这是scala语言自带的能力
        ...
        /**
          ...
          *         1）将函数分解成多个独立的参数
          *         2）使用currying，将函数变成多个参数的函数；因此对最后一个参数，可以使用 {}
          */
        if (true) {
            /**
            ...
            /**
              * 2）为了让有多个参数的函数，也能看起来像控制结构，可以使用currying后，调用时对最后一个参数使用 {}
              */
