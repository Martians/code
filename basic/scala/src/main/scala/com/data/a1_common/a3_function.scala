...
object a3_function extends App {
    ...
    def operator_func()= {
        ...
        /**
          ...
          *              println和_之间有一个空格，防止被当做 println_ 函数原型（防止编译器认为是定义的一个新函数名）
          *
          *           2) 如果目标位置正好是需要一个函数（而不是函数的执行结果）
          *              可以进一步简化, _可以省略
          *              注意：如果目标位置需要一个值，如println，那么就不能省略 _, 否则println出来的是 <function> 这个类型
          *                 println(list.tail), println(list.tail _)
          */
    ...
    /**
      ...
      *     3. 函数值是“关闭”这个行动的最终产物，得到的函数中将包含指向捕获变量的参考
      *
      */
    ...
    def operator_abstract(): Unit = {
        ...
        /**
          ...
          *     1. filesMatching：根据传入的高阶函数，返回不同的语义
        ...
        def filesMatching(matcher: String => Boolean) =
            for (file <- files; if matcher(file))
                yield file
        def filesEnding(query: String) =
            filesMatching(_.endsWith(query))

        val cc = filesEnding("abc")
        println("file match: " + cc)
        ...
        if (true) {
            ...
            // a的类型是一个函数 (Int) => Int
            /**
              * 第二个参数使用占位符，使用了偏应用函数的方式, 使用_代替了剩余的所有参数
        ...
        if (true) {
            ...
            /**
              * 2）为了让有多个参数的函数，也能看起来像控制结构，可以使用cury，对最后一个参数使用 {}
              */
        ...
        /**
          *    方式2：叫名参数 P134
          *         例子：实现一个类似assert的结构，在release版本下不启用
          *
